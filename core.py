...
class DebugUtils:
    ...
    def __init__(self, debug_mode=False):
        ...
        self.max_buffer_size = 5000  # 最大缓冲区大小
...
class ImageViewer:
    ...
    def view(self, img_path, parent=None):
        ...
        canvas = tk.Canvas(viewer, bg="#2b2b2b", highlightthickness=0)
        ...
        try:
            ...
            self._update_image(viewer, canvas, original_img, 1.0)
            ...
            drag_data = {"x": 0, "y": 0}
            def on_drag_start(e): drag_data.update(x=e.x, y=e.y)
            def on_drag_move(e):
                dx, dy = e.x - drag_data["x"], e.y - drag_data["y"]
                canvas.move("all", dx, dy)
                drag_data.update(x=e.x, y=e.y)
            ...
            info_label = ttk.Label(info_frame, 
                text=f"尺寸: {img_w}x{img_h} | 缩放: 100% | 滚轮缩放 拖拽移动 双击退出",
                relief=tk.SUNKEN, anchor=tk.W)
            info_label.pack(fill=tk.X)
            viewer.info_label = info_label
    ...
    def _update_image(self, viewer, canvas, img, zoom):
        w, h = img.size
        new_w = min(int(w * zoom), 4000)
        new_h = min(int(h * zoom), 4000)
        
        if new_w > 4000 or new_h > 4000:
            ratio = min(4000 / new_w, 4000 / new_h)
        ...
        if hasattr(viewer, 'info_label') and viewer.info_label.winfo_exists():
            viewer.info_label.config(text=f"尺寸: {w}x{h} | 缩放: {int(zoom * 100)}%")
    ...
    def _on_zoom(self, event, viewer):
        ...
        self._update_image(viewer, viewer.canvas, viewer.original_img, viewer.zoom_level)
